...
启用插件后可在底部面板打开 `GF Packages`，查看、启用或禁用官方包/社区包。禁用包不会自动执行自己的 Installer，并且在开启导出排除后不会进入导出产物。官方包之间不互相强依赖；完全不用的官方包也可以删除目录，但项目侧不能再直接引用被删除包里的脚本、场景或资源。禁用包仍被项目文件引用时，包管理面板和导出流程会给出警告；需要更严格导出策略时，可启用 `gf/packages/export_fail_on_disabled_references` 把审计结果报告为错误。
