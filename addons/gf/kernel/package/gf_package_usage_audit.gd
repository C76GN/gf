## GFPackageUsageAudit: 检查禁用包是否仍被项目文件直接引用。
class_name GFPackageUsageAudit
extends RefCounted
...
## 检查一组禁用包是否仍被项目文件直接引用。
## @param manifests: 要检查的禁用包 manifest 列表。
## @param options: 可选参数，支持 scan_roots、ignored_roots、max_references_per_package。
## @return 引用审计报告。
static func audit_disabled_packages(
	manifests: Array[GFPackageManifest],
	options: Dictionary = {}
) -> Dictionary:
	var package_reports: Dictionary = {}
	...
	return {
		...
		"package_count": package_reports.size(),
...
## 查找项目文件中对指定包根目录的直接路径引用。
## @param root_path: 包根目录。
## @param options: 可选参数，支持 scan_roots、ignored_roots、max_references_per_package。
## @return 引用列表。
static func find_references_to_root(root_path: String, options: Dictionary = {}) -> Array[Dictionary]:
	...
	var package_class_names := _collect_package_class_names(normalized_root)
	var max_references := maxi(int(options.get("max_references_per_package", 50)), 1)
...
static func _collect_package_class_names(root_path: String) -> Array[String]:
	var files: Array[String] = []
