...
func to_dict(json_compatible: bool = false) -> Dictionary:
	return {
		...
		"url": url,
		"body": GFVariantUtility.variant_to_json_compatible(body) if json_compatible else body.duplicate(true),
...
func apply_dict(data: Dictionary, json_compatible: bool = false) -> void:
	...
	var body_value: Variant = GFVariantUtility.json_compatible_to_variant(raw_body) if json_compatible else GFVariantUtility.duplicate_variant(raw_body)
	...
	var metadata_value: Variant = GFVariantUtility.json_compatible_to_variant(raw_metadata) if json_compatible else GFVariantUtility.duplicate_variant(raw_metadata)
...
static func from_dict(data: Dictionary, json_compatible: bool = false) -> RefCounted:
	var script := load("res://addons/gf/utilities/gf_request_envelope.gd") as Script
