...
func _restore_data(data: Variant) -> bool:
	if _restore_callback.is_valid():
		var result: Variant = _restore_callback.call(GFVariantUtility.duplicate_variant(data))
		return result != false
...
func _make_record(snapshot_id: int, data: Variant, metadata: Dictionary) -> Dictionary:
	return {
		...
		"data": GFVariantUtility.duplicate_variant(data),
	}
...
func _duplicate_record(record: Dictionary) -> Dictionary:
	return {
		...
		"metadata": GFVariantUtility.duplicate_variant(record.get("metadata", {})),
		"data": GFVariantUtility.duplicate_variant(record.get("data", null)),
	}
