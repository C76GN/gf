...
func set_value(key: StringName, value: Variant) -> void:
	state[key] = GFVariantUtility.duplicate_variant(value)
...
func make_delta_to(target: GFNetworkSnapshot) -> Dictionary:
	...
	for key: Variant in target.state.keys():
		if not state.has(key) or state[key] != target.state[key]:
			set_values[key] = GFVariantUtility.duplicate_variant(target.state[key])
	for key: Variant in state.keys():
		if not target.state.has(key):
			erase_keys.append(GFVariantUtility.duplicate_variant(key))
	return {
		"ok": true,
		"from_tick": tick,
		"to_tick": target.tick,
...
func apply_delta(delta: Dictionary) -> GFNetworkSnapshot:
	...
	if set_values != null:
		for key: Variant in set_values.keys():
			next_snapshot.state[key] = GFVariantUtility.duplicate_variant(set_values[key])
