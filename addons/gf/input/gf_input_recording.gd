...
func add_event(
	...
	var event := {
		...
		"action_id": action_id,
		"value": GFVariantUtility.duplicate_variant(value),
...
func get_events() -> Array[Dictionary]:
	...
	for event: Dictionary in events:
		result.append(GFVariantUtility.duplicate_variant(event) as Dictionary)
	return result
...
func to_dict(json_compatible: bool = false) -> Dictionary:
	...
	return {
		...
		"metadata": GFVariantUtility.variant_to_json_compatible(metadata) if json_compatible else metadata.duplicate(true),
	}
...
func apply_dict(data: Dictionary, json_compatible: bool = false) -> void:
	...
	metadata_value = GFVariantUtility.json_compatible_to_variant(metadata_value) if json_compatible else GFVariantUtility.duplicate_variant(metadata_value)
...
static func from_dict(data: Dictionary, json_compatible: bool = false) -> RefCounted:
	var script := load("res://addons/gf/input/gf_input_recording.gd") as Script
...
func _event_to_dict(event: Dictionary, json_compatible: bool) -> Dictionary:
	return {
		...
		"value": GFVariantUtility.variant_to_json_compatible(event.get("value")) if json_compatible else GFVariantUtility.duplicate_variant(event.get("value")),
		"player_index": int(event.get("player_index", -1)),
		"source_id": String(event.get("source_id", "")),
		"metadata": GFVariantUtility.variant_to_json_compatible(event.get("metadata", {})) if json_compatible else GFVariantUtility.duplicate_variant(event.get("metadata", {})),
	}


func _event_from_dict(event: Dictionary, json_compatible: bool) -> Dictionary:
	var value: Variant = event.get("value", null)
	value = GFVariantUtility.json_compatible_to_variant(value) if json_compatible else GFVariantUtility.duplicate_variant(value)
	var event_metadata: Variant = event.get("metadata", {})
	event_metadata = GFVariantUtility.json_compatible_to_variant(event_metadata) if json_compatible else GFVariantUtility.duplicate_variant(event_metadata)
	return {
