...
GF Framework 的核心基于控制反转 (IoC) 的思想，通过统一的调度器管理所有组件。整体分为四大核心层：**Model（数据模型）**, **System（系统逻辑）**, **Controller（控制器/表现）**, 和 **Utility（工具）**。

## 核心单例与体系结构

整个框架的入口是全局 AutoLoad 节点 —— **`Gf`**。它唯一的作用是挂载在 Godot 的全局根节点下，驱动 `_process` 与 `_physics_process`。

在 `Gf` 背后，真正承载所有业务的对象是 **`GFArchitecture`**。它是一个纯代码容器，负责管理所有 `Model`、`System`、`Utility` 的注册、生命周期调用以及事件总线的派发。
...
### 3. Controller (表现层/控制器) - `GFController`
