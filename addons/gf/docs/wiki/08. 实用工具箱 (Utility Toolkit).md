...
`GFTouchJoystick` 是一个可直接放进场景树的 `Node2D`。它会发出 `direction_changed(direction)`，也可以把方向映射到项目自己的 InputMap 动作名。框架只处理方向和动作映射，不规定移动、瞄准或 UI 导航规则。
