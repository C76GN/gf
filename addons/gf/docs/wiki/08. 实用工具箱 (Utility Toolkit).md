...
项目可以继承 `GFSaveSource` 或注册自定义 `GFNodeSerializer`，也可以在需要补建实体时注册 `GFSaveEntityFactory`。默认能力提供 Transform、CanvasItem、Control、Range 等通用节点状态片段，并可通过 `GFSavePipelineStep` 在采集/应用前后插入校验、版本适配或调试标记；具体 schema 仍由项目层决定。
...
命令补全可按前缀查询，未知命令也可通过 `suggest_similar_commands()` 给出相似候选，便于调试控制台在输入错误时提示可用命令。
