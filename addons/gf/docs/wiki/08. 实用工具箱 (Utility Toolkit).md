...
> 提示：本页只讨论需要注册到 `Gf.register_utility(...)`、会参与生命周期或持有运行时状态的 `Utility`。像大数、定点数、数值显示格式化这类纯基础件，请改看 [[11. 基础层 (Foundation Layer)]]。
...
## 7. 输入与土狼时间 (`GFInputUtility` / `GFInputMappingUtility`)
...
## 13. 内部通信控制台 (`GFDebugOverlayUtility`)
...
## 16. 通用的静态导表数据适配器 (`GFConfigProvider`)
...
## 21. 远程文本与 JSON 缓存 (`GFRemoteCacheUtility`)
