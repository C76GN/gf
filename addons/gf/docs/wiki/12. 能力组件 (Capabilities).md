...
启用 GF 插件后，选中普通 `Node` 时 Inspector 会显示 `GF Capabilities` 区域。这里可以添加、启停、编辑和移除继承 `GFNodeCapability` 的能力脚本或能力场景，并可直接编辑能力导出属性。编辑器菜单也提供 `工具 > GF > 生成 Capability` 与 `工具 > GF > 生成 NodeCapability` 模板入口。
