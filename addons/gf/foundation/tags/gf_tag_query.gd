...
const _GF_TAG_UTILITY_SCRIPT: Script = preload("res://addons/gf/foundation/tags/gf_tag_utility.gd")
...
func get_match_report(source: Variant) -> Dictionary:
	...
	for tag: StringName in all_tags:
		if not _GF_TAG_UTILITY_SCRIPT.source_has_tag(source, tag, 1, include_child_tags):
	...
	for tag: StringName in none_tags:
		if _GF_TAG_UTILITY_SCRIPT.source_has_tag(source, tag, 1, include_child_tags):
