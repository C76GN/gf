...
func build_source(records: Array) -> String:
	...
	builder.blank(2)
	builder.section("常量")
	builder.line("const _CAPABILITY_UTILITY_SCRIPT: Script = preload(\"res://addons/gf/extensions/capability/gf_capability_utility.gd\")")
	builder.blank(2)
	...
	builder.line("return null")
	builder.dedent()
	builder.line("return resolved_architecture.get_utility(_CAPABILITY_UTILITY_SCRIPT)")
	builder.dedent()
	builder.blank(2)
