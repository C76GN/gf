...
插件还会打开 `GF Packages` 底部面板。你可以用它查看包 manifest、启用或禁用 GF 包、自动运行启用包的安装器、在导出时排除禁用包目录，并在需要时让禁用包引用导致导出检查失败。

官方包之间没有硬依赖。未使用的包可以禁用、从导出中排除，或在项目脚本、场景、资源和 preload 都不再引用后直接移除。
...
- `addons/gf/kernel`：运行内核、基础契约、架构容器、绑定、事件、命令、查询、工厂、AutoLoad 入口、包基础设施和核心编辑器集成。
- `addons/gf/standard`：稳定标准库，包括 foundation、input、utilities、状态机、命令历史、序列辅助和 common 支撑原语。
- `addons/gf/packages/official`：随 GF 发布的可选官方包，例如 capability、interaction、feedback、action queue、combat、save、flow、network、turn-based、behavior tree、physics 辅助和 domain model。
- `addons/gf/packages/community`：项目本地或第三方包的约定目录，这些包应遵循 GF 包 manifest 和目录规范。

`kernel` 不硬引用 `standard` 或可选包。`standard` 只依赖 `kernel`，不能通过包 ID、路径、动态加载或包内类名探测官方包。需要出现在标准库诊断或工具里的包能力，应由包侧通过通用注册 API 主动贡献，或由项目显式装配。

## 编辑器工具

GF 提供包管理、强类型 GF/config 访问器生成、项目常量、脚本模板、Inspector、Dock、导出辅助、SaveGraph 诊断、Pattern2D 编辑和 Node3D/Mesh/MeshLibrary 缩略图渲染等编辑器能力。

包专属编辑器工具由 `gf_package.json` manifest 声明，并且只会在包启用时加载。
