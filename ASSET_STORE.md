...
- Release tag: `4.1.0`
- Minimum Godot version: `4.6`
- Download package should use the attached `gf-framework-4.1.0.zip` release asset generated by `tools/build_asset_store_package.py`, not GitHub's automatic source archive.
- Package layout must have `addons/` at the zip root, with the plugin under `addons/gf/**`.
- Release metadata check: `python tools\gf_maintenance.py release-status --version 4.1.0`
