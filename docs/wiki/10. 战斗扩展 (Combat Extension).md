...
`attribute_id` 表示这个修饰器要挂到哪一个属性上；`source_id` 表示它来自哪个装备、Buff 或技能，便于按来源批量移除。旧项目只填写 `source_tag` / `source_id` 时，Buff 仍会把它作为目标属性兼容回退并输出 warning；新代码应始终显式填写 `attribute_id`。
