...
1. **项目级 Installer**：继承 `GFInstaller` 并重写 `install(architecture)`，再把脚本路径加入 `Project Settings > gf/project/installers`。`Gf.init()` 与 `Gf.set_architecture()` 会在三阶段生命周期开始前自动执行这些 Installer，适合集中注册全局 Model、System 和 Utility。默认情况下，配置错误会输出错误并跳过该 Installer；如果希望这类错误直接中断初始化，可开启 `gf/project/fail_on_installer_error`。
