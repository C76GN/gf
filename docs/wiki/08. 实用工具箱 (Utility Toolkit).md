...
- **资源、对象与流程**：`GFAssetUtility` 负责异步资源缓存，`GFDownloadUtility` / `GFDownloadTask` 负责通用文件下载队列，`GFObjectPoolUtility` 负责节点复用，`GFSceneUtility` 负责场景切换与瞬态模块清理，`GFUIUtility` 负责分层 UI 栈。
- **时间与信号**：`GFTimerUtility` 提供受 GF 逻辑时间影响的延迟、重复和 owner 绑定回调，`GFTimeUtility` 管理全局或分组时间缩放，`GFSignalUtility` 处理 Godot 原生信号的 owner 清理、一次性监听、过滤和防抖。
- **存档、配置与设置**：`GFStorageUtility` / `GFStorageCodec` 负责本地持久化字典、槽位和 `Resource`，`GFSaveGraphUtility` 负责节点状态采集与恢复，`GFSettingsUtility` / `GFDisplaySettingsUtility` 负责抽象设置及其引擎应用，`GFConfigProvider` 负责静态导表读取，`GFConfigTableColumn` / `GFConfigTableSchema` / `GFConfigTableImporter` 负责通用导表结构声明、导入和校验。
...
- **调试与运行时运维**：`GFLogUtility` 负责集中日志，`GFConsoleUtility` 负责运行时开发者控制台，`GFDiagnosticsUtility` 负责诊断快照和诊断命令，`GFNotificationUtility` 只提供通知队列，`GFDebugOverlayUtility` 显示模型状态与项目主动注册的轻量 watch 覆盖层。
...
切换期间如果注册了 `GFTimeUtility`，工具会暂时设置全局暂停，并在成功或失败后恢复旧暂停状态。带 loading scene 的失败恢复依赖上一场景的 `scene_file_path`；如果当前场景没有资源路径，工具会跳过 loading scene，避免失败后无法切回。Loading scene 可以选择实现 `fade_in()`、`fade_out()`、`set_progress(progress)` 或 `update_progress(progress)`，GF 会在切入、进度变化和退出前按约定调用；这些方法只是协议钩子，不规定 UI 样式、文案或动画。GF 只管理场景资源生命周期、切换、进度信号和瞬态模块清理；具体加载界面表现、场景内初始化和关卡解锁规则仍属于项目层。
...
`GFConfigTableImporter` 提供轻量 JSON/CSV 文本解析和 `validate_json_table()` / `validate_csv_table()` 入口，适合编辑器导入按钮、CI 检查或项目自定义导表流水线在写入缓存前做统一报告。校验报告固定包含 `ok`、`row_count`、`error_count`、`warning_count` 和 `issues`，项目工具可以直接把 `issues` 渲染成表格或控制台输出。

开发期如果需要做 Resource 批量检查或表格式编辑，可以复用 `GFResourceTableEditor` 和 `GFEditorValueField`。前者负责扫描 `.tres` / `.res`、从 Resource export 推导列、提交单元格值并广播变更；后者负责按 Godot 属性类型创建基础输入控件。它们是编辑器通用控件，不会保存业务表结构，也不替项目决定资源分类、校验规则或提交工作流。
...
诊断快照的 `tools` 字段会聚合已注册模块公开的 `get_debug_snapshot()`，目前会自动尝试读取 `GFAssetUtility`、`GFTimerUtility`、`GFRemoteCacheUtility`、`GFDownloadUtility`、`GFObjectPoolUtility` 和 `GFActionQueueSystem`。这些快照只表达队列、缓存、pending 数量和运行状态，不解释项目业务含义。
...
缓存文件位于 `user://<cache_dir_name>/`，文件名由 URL 的 MD5 派生，超过 `max_cache_entries` 后按修改时间删除最旧条目。项目可以用 `has_valid_cache()` / `get_cached_text()` 只读缓存，用 `remove_cache()` 清理单个 URL，用 `clear_cache()` 清空整个缓存目录。`get_debug_snapshot()` 会报告缓存目录、TTL、队列数量和当前 active URL，便于和 `GFDiagnosticsUtility` 一起定位远程配置刷新问题。该工具串行处理内部请求队列，适合轻量公告和配置拉取，不适合作为大文件下载器或实时 API 客户端。
