...
- **资源、对象与流程**：`GFAssetUtility` / `GFAssetHandle` 负责异步资源缓存、引用句柄与资源分组，`GFDownloadUtility` / `GFDownloadTask` 负责通用文件下载队列，`GFJobQueueUtility` / `GFJob` 负责通用任务排队和进度状态，`GFObjectPoolUtility` 负责节点复用，`GFSceneUtility` 负责场景切换、后台预加载、切换参数、场景历史与瞬态模块清理，`GFUIUtility` 负责分层 UI 栈。
...
- **存档、配置与设置**：`GFStorageUtility` / `GFStorageCodec` 负责本地持久化字典、槽位和 `Resource`，`GFStorageBackend` / `GFStorageConflictReport` 提供可选后端扩展协议和同步冲突 DTO，`GFSaveGraphUtility` 负责节点状态采集与恢复，`GFSettingsUtility` / `GFDisplaySettingsUtility` 负责抽象设置及其引擎应用，`GFConfigProvider` 负责静态导表读取，`GFConfigTableColumn` / `GFConfigTableSchema` / `GFConfigTableImporter` 负责通用导表结构声明、导入和校验，`GFConfigAccessGenerator` 负责生成可选静态导表访问器。
...
`GFStorageBackend` 是可选的后端扩展接口，默认不参与 `GFStorageUtility` 的本地读写流程，避免把云同步、平台 SDK 或账号体系写进框架核心。项目需要多端同步时，可以继承后端接口并在自己的存储系统里组合使用；遇到本地/远端字段冲突时，用 `GFStorageConflictReport` 描述 `file_name`、`key`、本地值、远端值、解决结果和元数据。GF 只提供报告 DTO，不替项目决定“本地优先、远端优先、合并或跳过”。
