...
- **资源、对象与流程**：`GFAssetUtility` / `GFAssetHandle` 负责异步资源缓存、引用句柄与资源分组，`GFDownloadUtility` / `GFDownloadTask` 负责通用文件下载队列，`GFJobQueueUtility` / `GFJob` 负责通用任务排队和进度状态，`GFJobWorker` 可作为场景节点按批次消费队列，`GFObjectPoolUtility` 负责节点复用，`GFSceneUtility` 负责场景切换、后台预加载、切换参数、场景历史与瞬态模块清理，`GFUIUtility` 负责分层 UI 栈。
...
- **输入、音频与关卡**：`GFInputUtility` 处理动作缓冲和土狼时间，`GFInputMappingUtility` 处理资源化动作映射，`GFInputDeviceUtility` 处理本地多人设备席位，`GFInputDeviceTextProvider` 处理通用手柄文本，`GFAudioUtility` 处理 BGM/SFX/环境音，`GFAudioBankMounter` 处理场景级音频集合挂载，`GFLevelUtility` 处理通用关卡生命周期。
...
- **调试与运行时运维**：`GFBuildInfoUtility` 负责构建信息快照，`GFBuildInfoExportPlugin` 可在导出时写入 Git 构建字段，`GFLogUtility` 负责集中日志，`GFConsoleUtility` 负责运行时开发者控制台，`GFDiagnosticsUtility` 负责诊断快照和诊断命令，`GFNotificationUtility` 只提供通知队列，`GFDebugOverlayUtility` 显示模型状态与项目主动注册的轻量 watch 覆盖层。
