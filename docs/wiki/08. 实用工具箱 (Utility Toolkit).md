...
- **输入、音频与关卡**：`GFInputUtility` 处理动作缓冲和土狼时间，`GFInputMappingUtility` 处理资源化动作映射，`GFInputDeviceUtility` 处理本地多人设备席位，`GFInputDeviceTextProvider` / `GFInputIconAtlasProvider` 处理通用输入文本与可配置图标映射，`GFAudioUtility` 处理 BGM/SFX/环境音，`GFAudioBankMounter` 处理场景级音频集合挂载，`GFLevelUtility` 处理通用关卡生命周期。
