...
如果已经注册了分段迁移，旧存档版本到当前 `save_version` 必须能解析出完整链路；缺失某一段时读取会失败并发出 `data_integrity_failed`，不会把旧结构伪标记成当前版本。只依赖 `default_values_for_new_keys` 补齐字段、且没有注册迁移步骤的项目仍可继续使用默认迁移路径。
