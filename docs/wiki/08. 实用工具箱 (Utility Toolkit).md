...
GF Framework 的核心极具克制，但针对实际 Godot 开发中屡屡遇到的通用需求问题，提供了一组被广泛验证过的"开箱即用"运行时工具组件。源码按职责归位：通用运行时服务放在 `addons/gf/standard/utilities/<domain>/`，输入体系放在 `addons/gf/standard/input/<domain>/`，更偏可选能力包的玩法领域与 AI 辅助放在 `addons/gf/packages/official/<package>/`。
