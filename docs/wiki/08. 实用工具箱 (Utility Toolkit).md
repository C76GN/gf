...
- **资源、对象与流程**：`GFAssetUtility` 负责异步资源缓存，`GFDownloadUtility` / `GFDownloadTask` 负责通用文件下载队列，`GFObjectPoolUtility` 负责节点复用，`GFSceneUtility` 负责场景切换、切换参数、场景历史与瞬态模块清理，`GFUIUtility` 负责分层 UI 栈。
...
- **调试与运行时运维**：`GFBuildInfoUtility` 负责构建信息快照，`GFLogUtility` 负责集中日志，`GFConsoleUtility` 负责运行时开发者控制台，`GFDiagnosticsUtility` 负责诊断快照和诊断命令，`GFNotificationUtility` 只提供通知队列，`GFDebugOverlayUtility` 显示模型状态与项目主动注册的轻量 watch 覆盖层。
...
`use_integrity_checksum` 会在新写入载荷中写入 `_meta.checksum` 并在读取时校验；为了兼容旧存档，默认缺少 checksum 的载荷仍会被视为可读。需要把“缺少 checksum”也视为失败时，同时启用 `strict_integrity` 和 `require_integrity_checksum`。JSON 读取默认会把接近整数的 float 归一为 int，保持旧版本存档列表和元数据的整数语义；如果项目需要严格保留 JSON 数字类型，可把 `GFStorageUtility.normalize_json_numbers` 或 `GFStorageCodec.normalize_json_numbers` 设为 `false`。`allow_absolute_paths` 在 1.x 中默认仍为 `true` 以兼容已有工具链；运行时接收用户输入或外部配置路径时，建议显式设为 `false`，把绝对路径收敛回 `user://<save_dir_name>/` 下的同名文件。

`save_slot()` 只接受大于等于 0 的整数槽位；`load_slot_result()` / `load_slot_meta_result()` 可区分“合法空字典”和“文件缺失、非法槽位或解码失败”。异步 `save_data_async()` / `load_data_async()` 会按文件串行和线程预算调度；如果同一路径需要混合同步和异步读写，先调用 `wait_for_async_tasks()` 收敛已入队任务顺序，再执行同步 `save_data()` / `load_data()`。`dispose()` 会等待已开始的线程结束并发出对应完成信号，对尚未开始的队列任务发出失败结果，避免调用方一直等待完成通知。项目如需复杂迁移，可继承 `GFStorageUtility` 并重写 `migrate_data(data, from_version, to_version)`。插件启用后也会提供 `GF Save Viewer` Dock，用于按 codec 选项查看本地存档内容、校验状态并复制 JSON，方便调试而不绑定任何项目业务结构。
...
`GFInputAction.ValueType` 支持 `BOOL`、`AXIS_1D`、`AXIS_2D` 与 `AXIS_3D`。`GFInputBinding.ValueTarget.AUTO` 会按动作值类型自动产出贡献值，但二维/三维动作默认仍写入 X 分量；摇杆 Y、右摇杆、Z 轴或按钮方向应使用显式 `AXIS_2D_*` / `AXIS_3D_*` 目标。旧的 `get_action_vector()` / `get_action_vector_for_player()` 继续返回 `Vector2`；需要三维输入时使用 `get_action_vector3()` 或 `get_action_vector3_for_player()`。
...
切换参数可直接传给 `load_scene_async(path, loading_scene_path, params, minimum_duration_seconds)`，也可以写在 `GFSceneTransitionConfig.params` 中。切换成功后，`get_current_scene_params()` 返回当前场景参数副本，适合场景入口脚本读取出生点、入口来源、过场配置或项目自定义 DTO。`minimum_duration_seconds` 只控制 loading scene 至少停留多久，避免缓存命中时过渡 UI 一闪而过；它不替代目标场景自己的初始化等待。
...
`get_state()` / `set_state()` 只处理主 RNG 的内部状态；如果项目还使用了 `get_branched_rng()`，应使用 `get_full_state()` / `set_full_state()` 保存和恢复主种子、主 RNG 状态以及各标签的分支调用计数。`set_global_seed()` 会重置分支计数。作为 Utility 注册到架构时会正常初始化；测试或工具代码直接 `GFSeedUtility.new()` 调用公共方法时，也会懒初始化内部 RNG。
...
`GFBuildInfo` 是轻量 Resource，包含项目名、项目版本、GF 版本、构建号、提交号、分支、构建时间、Godot 版本、平台和自定义 `metadata`。`GFBuildInfo.collect()` 会从 `ProjectSettings` 与 `addons/gf/plugin.cfg` 采集当前环境；项目发布流水线可以写入 `gf/build/id`、`gf/build/commit_hash`、`gf/build/branch`、`gf/build/time_utc` 与 `gf/build/metadata`，运行时再统一读取。
...
诊断快照的 `tools` 字段会聚合已注册模块公开的 `get_debug_snapshot()`，目前会自动尝试读取 `GFBuildInfoUtility`、`GFAssetUtility`、`GFTimerUtility`、`GFRemoteCacheUtility`、`GFDownloadUtility`、`GFObjectPoolUtility` 和 `GFActionQueueSystem`。这些快照只表达版本、队列、缓存、pending 数量和运行状态，不解释项目业务含义。快照默认可包含构建信息、最近日志、网络状态、URL 派生的缓存状态、工具路径和项目自定义 monitor 输出；如果要暴露给远程调试、玩家可访问控制台或线上 GM 工具，应在项目层做脱敏、白名单过滤和权限控制。
...
`push_notification()` 可通过 `options` 设置 `duration_seconds`、去重 `key` 和项目自定义 `metadata`；返回值是通知 ID，被重复抑制时会返回已有通知 ID。显式传入 `key` 时只按 key 去重；没有 key 时才按消息文本去重，因此不同业务上下文可以显示相同正文。`max_queue_size = 0` 表示只保留当前通知，不保留等待队列。`duration_seconds <= 0` 的通知会在开始后立刻以 `timeout` 结束，监听 UI 时不要假设它至少展示一帧。`tick()` 会推进当前通知时长，`dismiss_active()` 和 `clear_notifications()` 可用于玩家手动关闭或切换场景时清空队列。
