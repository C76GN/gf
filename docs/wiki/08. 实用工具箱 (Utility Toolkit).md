...
诊断快照的 `tools` 字段会聚合已注册模块公开的 `get_debug_snapshot()`，目前会自动尝试读取 `GFAssetUtility`、`GFTimerUtility`、`GFRemoteCacheUtility`、`GFDownloadUtility`、`GFObjectPoolUtility` 和 `GFActionQueueSystem`。这些快照只表达队列、缓存、pending 数量和运行状态，不解释项目业务含义。快照默认可包含最近日志、网络状态、URL 派生的缓存状态、工具路径和项目自定义 monitor 输出；如果要暴露给远程调试、玩家可访问控制台或线上 GM 工具，应在项目层做脱敏、白名单过滤和权限控制。
