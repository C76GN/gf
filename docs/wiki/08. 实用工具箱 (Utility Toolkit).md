...
同一路径的并发加载会合并到同一个 threaded request；如果已存在请求或缓存的资源类型与新的 `type_hint` 明显不兼容，回调会收到 `null`。工具只管理 `ResourceLoader` 请求、回调分发和内存缓存，不负责实例化节点、引用计数之外的资源生命周期或远程下载。
...
`execute_after()` 只提供一次性延迟任务；`delay <= 0` 时会立即执行回调。它没有返回句柄，也不提供单独取消某个计时器的接口；需要可取消任务时，项目层应在回调里检查自己的 token 或状态。框架 `dispose()` 时会清空尚未触发的任务。
...
连接会用弱引用追踪 owner，`prune_invalid_connections()` 会清理 owner 或信号源已经失效的连接。`delay()` / `debounce()` 使用 SceneTree 计时器或帧等待做信号层延迟，适合 UI 防抖和输入节流；它不是 `GFTimerUtility` 的替代品，也不会表达 GF 逻辑时间组的暂停语义。
...
归还时节点会被移动到内部 `GFObjectPoolRoot`，并恢复/关闭 `process_mode`、`CanvasItem.visible` 和常见 `disabled` 属性；`manage_descendant_active_state` 控制是否递归处理子节点。`release()` 会校验节点是否确实来自对应池，避免把外部节点或其他 `PackedScene` 的实例混入。`get_available_count()`、`get_active_count()`、`get_debug_snapshot()` 可用于调试池容量。
...
`configure(false)` 可以关闭“压入新面板时自动隐藏同层旧顶层面板”的行为。`pop_panel(layer, false)` 会把面板从 UI 根节点移除但不释放，适合项目层自己复用实例；如果面板被外部 `queue_free()`，工具会在 `tree_exited` 后从栈中移除并恢复下层面板。`push_panel_async()` 会优先使用 `GFAssetUtility`，未注册时回退同步加载。
...
`get_scene_resource_state()` 可区分未加载、预加载中、已缓存和当前加载。预加载请求可用 `cancel_scene_preload()` 或 `cancel_all_scene_preloads()` 标记取消；已缓存的场景可用 `remove_preloaded_scene()` 或 `clear_preloaded_scenes()` 手动释放。
...
默认总线名为 `BGM` / `SFX`，找不到时会回退到 `Master` 并按总线名只警告一次。`GFAudioClip` 可描述 stream/path、bus、音量和 pitch；`GFAudioBank` 只是 clip 的资源化集合，不负责随机规则、权重或状态机。复杂混音、音频快照和平台音频权限仍属于项目层。
...
它不是命令行控制台，也不执行指令；需要输入调试命令、查看日志输出时请使用后面的 `GFConsoleUtility`。
...
flush 失败时，本批事件会按原顺序放回队列前端，并发出 `flush_failed` / `flush_completed`；队列超过 `max_queue_size` 时会丢弃最早事件，避免离线或接口故障时无限占用内存。`capture_context()` 只采集平台、Godot 版本、屏幕尺寸、语言和时区等通用信息，涉及账号、设备指纹或隐私字段的内容必须由项目层显式添加。
