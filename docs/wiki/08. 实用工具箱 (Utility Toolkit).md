...
- **输入、音频与关卡**：`GFInputUtility` 处理动作缓冲和土狼时间，`GFInputMappingUtility` / `GFVirtualInputSource` / `GFInputRecording` / `GFInputPlayback` 处理资源化动作映射、可编程虚拟输入源和抽象动作录制回放，`GFInputSequenceBranch` / `GFInputSequenceStep` 描述可复用动作序列，`GFInputDeviceUtility` 处理本地多人设备席位，`GFInputDeviceTextProvider` / `GFInputIconAtlasProvider` 处理通用输入文本与可配置图标映射，`GFTextFitUtility` 提供轻量文本尺寸适配，`GFRichTextFormatter` 提供安全 BBCode 格式化辅助，`GFAudioUtility` 处理 BGM/SFX/环境音，`GFAudioBankMounter` 处理场景级音频集合挂载，`GFLevelUtility` 处理通用关卡生命周期。
...
### 通用节点树辅助 (`GFNodeTreeUtility`)

`GFNodeTreeUtility` 是纯静态节点树辅助类，不需要注册到 `GFArchitecture`。它适合编辑器工具、运行时装配、能力容器、对象池或场景工厂中复用一些容易写散的操作：安全添加子节点并设置 `owner`、重挂节点、替换子节点、按类型向上/向下查找、收集节点树、递归设置 owner 和释放直接子节点。

```gdscript
var capability := HitboxCapability.new()
GFNodeTreeUtility.add_child_with_owner(container, capability, get_tree().current_scene)

var camera := GFNodeTreeUtility.find_first_child_of_type(root, Camera3D, true) as Camera3D
var all_controls := GFNodeTreeUtility.collect_node_tree(root, Control)
```
...
`GFRequestEnvelope` 保存 `method`、`url`、`body`、`headers`、`idempotency_key`、`attempt_count`、`max_attempts`、`last_error` 和 `metadata`。队列写入 `storage_path` 时会使用 `GFVariantUtility` 的类型化 JSON codec，因此 `Vector2`、`Color`、PackedArray 等常见 Godot 值可以作为普通载荷保存。`transport_callback` 返回 `{ "ok": true }` 或 `{ "success": true }` 时请求会从等待队列移除；失败时按 `retry_delays_msec` 安排下一次尝试，耗尽次数后可进入失败列表。
