...
addons/gf/foundation/
 ...
 │   └── gf_fixed_decimal.gd
 ├── reflection/
 │   └── gf_script_type_utility.gd
 ├── tags/
 │   ├── gf_tag_query.gd
 │   ├── gf_tag_set.gd
 │   └── gf_tag_utility.gd
 ├── variant/
 │   └── gf_variant_utility.gd
 └── validation/
     ├── gf_validation_issue.gd
     ├── gf_validation_report.gd
     └── gf_validation_utility.gd
```
...
```gdscript
var rounded := GFDecimalStringUtility.format_decimal_value(12.345, 2, false, false)
var truncated := GFDecimalStringUtility.format_decimal_value(12.345, 2, false, true)
var valid := GFDecimalStringUtility.is_valid_decimal_parts("12", "34", true)
```
...
### `GFTagSet` / `GFTagQuery` / `GFTagUtility`
...
`GFTagUtility` 可读取 `GFTagSet`、`Array`、`PackedStringArray`、`Dictionary`，也可读取实现了 `has_tag()` / `get_tag_count()` / `get_tags()` 的对象。层级匹配只使用点号前缀，例如查询 `state` 时可匹配 `state.burning`；是否采用这种命名规范由项目层决定。
...
通用 Variant 辅助函数，用于框架内部和项目层复用深拷贝、默认值合并以及 JSON 友好的 Godot 类型转换。它不依赖 `GFArchitecture`，适合存档、配置、校验报告、网络消息、命中上下文等需要“复制集合但保留标量语义”的地方。
...
var copy := GFVariantUtility.duplicate_variant(payload) as Dictionary
...
GFVariantUtility.deep_merge_defaults(settings, {
	"audio": {
		"mute": false,
	},
	"language": "zh",
...
var saved_position := GFVariantUtility.vector2_to_array(Vector2(12.0, 4.0))
var position := GFVariantUtility.array_to_vector2(saved_position)

var json_payload := GFVariantUtility.variant_to_json_compatible({
	"position": Vector3(1.0, 2.0, 3.0),
	"tags": PackedStringArray(["state.ready"]),
})
var restored := GFVariantUtility.json_compatible_to_variant(
	JSON.parse_string(JSON.stringify(json_payload))
) as Dictionary
```

`variant_to_json_compatible()` 会为 `Vector2/3/4`、整数向量、`Color`、`Rect2`、`Transform2D/3D`、`Basis`、`Quaternion`、`AABB`、`Plane`、`NodePath`、`StringName` 和常见 PackedArray 写入轻量类型标记，再由 `json_compatible_to_variant()` 恢复。默认普通 Dictionary 仍使用字符串键；如果确实需要保留非字符串键，可传 `{ "encode_dictionary_keys": true }`。

`duplicate_variant()` 默认只深拷贝 `Dictionary` 和 `Array`，其他值保持原样返回；如果值中包含 `Object` 或 `Resource`，仍是引用语义。需要复制资源值时，可显式传入 `duplicate_variant(value, true, true)`，框架内部的权重表复制会使用这个模式保留资源化候选项的深拷贝语义。JSON codec 遇到不支持的对象默认写成 `null`，需要持久化对象时，应在项目层先转换成资源路径、ID 或纯数据字典。
...
```gdscript
if GFScriptTypeUtility.script_extends_or_equals(player_script, GFNodeCapability):
	print("This script is a node capability.")

var chain := GFScriptTypeUtility.get_inheritance_chain(player_script)
```

### `GFValidationIssue` / `GFValidationReport` / `GFValidationUtility`
...
已有模块如果仍返回字典报告，可以先使用 `GFValidationUtility.append_issue()` 和 `GFValidationUtility.finalize_report()` 统一统计字段，而不必立刻迁移成对象式报告：
...
GFValidationUtility.append_issue(legacy_report, "warning", &"missing_optional", "Optional field is missing.", {
	"row_key": 1,
})
GFValidationUtility.finalize_report(legacy_report, "Config table")
```
...
`GFResultUtility` 是更轻量的结果字典辅助，只统一常见字段名和基础构造，不负责统计问题或表达校验报告。它适合 `ok` / `data` / `metadata` / `error` 这类底层 Utility 返回值，尤其是存储、导入导出、异步任务等需要保持字典兼容的场景。

```gdscript
var result := GFResultUtility.make_success({
	GFResultUtility.KEY_DATA: { "coins": 10 },
	GFResultUtility.KEY_METADATA: { "version": 2 },
})

if result[GFResultUtility.KEY_OK]:
	print(result[GFResultUtility.KEY_DATA])
```

如果返回结构需要问题列表、严重级别、摘要和下一步建议，应使用 `GFValidationIssue` / `GFValidationReport` / `GFValidationUtility`；如果只是表达一次操作的成功、失败和载荷，优先复用 `GFResultUtility` 的 key 常量和轻量工厂，避免不同模块手写字段名漂移。
