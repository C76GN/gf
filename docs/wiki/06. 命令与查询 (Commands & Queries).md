...
工厂默认生命周期是 `GFBindingLifetimes.Lifetime.TRANSIENT`，每次解析都会创建新对象，适合携带参数的 Command / Query。需要缓存昂贵的无状态执行器时，可以用 `register_factory(..., GFBindingLifetimes.Lifetime.SINGLETON)` 或 `register_factory_instance()`，但不要把带有本次执行参数的命令注册成 singleton。
