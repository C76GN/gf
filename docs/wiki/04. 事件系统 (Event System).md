...
2. **优先使用 owner 绑定监听**：`GFSystem`、`GFUtility`、`GFController` 内部优先调用基类 `register_event()` / `register_simple_event()`；普通对象使用 `Gf.listen_owned()` / `Gf.listen_simple_owned()` 并在退出时调用 `Gf.unlisten_owner()`。如果使用无 owner 的 `Gf.listen()` / `Gf.listen_simple()`，必须手动 `unlisten`。
