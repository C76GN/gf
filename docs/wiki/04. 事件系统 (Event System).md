...
9. **需要时启用深度保护与追踪**：`TypeEventSystem.max_dispatch_depth` 默认 `0` 表示不限制，以保持旧项目语义；调试递归事件链或接入第三方模块时，可以设置上限避免无限嵌套。`trace_enabled` 默认关闭，开启后可通过 `Gf.get_event_dispatch_trace()` 读取最近派发条目，包括轨道、事件标识、监听数量、深度和时间戳；生产环境只建议在诊断面板或临时排查中开启。
