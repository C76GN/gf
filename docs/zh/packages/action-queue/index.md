...
本页聚焦 ActionQueue 包中的视觉动作、复合动作、命名队列、拦截器和反馈动作。
...
Action Queue 位于 `addons/gf/packages/official/action_queue/`。动作可以继承 `GFVisualAction`，也可以实现动作协议方法，例如 `execute()`、`can_execute()` 和 `cancel()`。
...
需要把相机抖动、UI 冲击或节点偏移纳入表现队列时，可以把 Feedback 包中的 `GFShakeAction` 放入 Action Queue。反馈采样、接收器和 `GFShakeUtility` 的主说明见 [Interaction 与 Feedback](../interaction-feedback/index.md)。
