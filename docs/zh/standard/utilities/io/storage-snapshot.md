...
本页聚焦标准库的本地读写、编码、同步和快照历史。场景树存档图属于 GF Save 扩展。
...
如果需要采集和恢复场景树节点状态，使用 GF Save 扩展的 `GFSaveGraphUtility` / `GFSaveScope`。标准库文档聚焦本地读写、编码、同步和快照能力；SaveGraph 的节点序列化器、槽位工作流和 pipeline trace 见 [Save 场景存档图](../../../extensions/save-graph/index.md)。
