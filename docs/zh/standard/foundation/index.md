...
这些类型常被多个模块同时使用，但它们不应该依赖运行时容器、场景树或可选包。把它们放在 `standard/foundation` 可以避免 `kernel` 膨胀，也能避免各个 Utility 重复实现基础能力。
