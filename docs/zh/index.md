...
- **扩展能力可选**：Capability、Save、Combat、Network、Flow、ActionQueue 等通用能力以 GF 内置原子扩展形式随框架发布，可启用、禁用或从导出中排除；跨扩展组合留给项目代码或独立插件。
...
- 通用但并非所有项目都需要的内置原子能力进 `extensions`；项目组合和第三方扩展进项目代码或 `addons/gf` 外的独立插件。
...
12. [GF 内置扩展总览与扩展规范](extensions/index.md)：manifest、启用状态、扩展 Installer、导出排除、外部扩展规范。
13. [编辑器工具、访问器与项目常量](editor/index.md)：GF 插件、扩展管理器、Inspector、工作区页面、导出插件和代码生成。
...
- [GF 内置扩展总览与扩展规范](extensions/index.md)
...
- `GF Workspace`：独立编辑器工作区，内部包含扩展管理、输入映射、存档文件查看、场景存档图、信号诊断和诊断快照等页面。
