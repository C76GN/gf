...
**Godot 4 轻量级架构框架：关注点分离、生命周期管理、事件驱动、标准库与可选包生态。**
...
- **包能力可选**：Capability、Save、Combat、Network、Flow、ActionQueue 等通用能力以官方包形式随 GF 发布，可启用、禁用或从导出中排除。

## 当前源码结构

```text
addons/gf/
  kernel/              # 运行内核、基础契约、架构容器、事件、绑定、包基础设施、核心编辑器装配
  standard/            # 稳定标准库：foundation、input、utilities、state_machine、sequence 等
  packages/
    official/          # 随 GF 发布的可选通用包
    community/         # 项目本地或社区包约定入口
```
...
12. [官方包总览与包规范](packages/index.md)：manifest、启用状态、包 Installer、导出排除、社区包规范。
13. [编辑器工具、访问器与项目常量](editor/index.md)：GF 插件、包管理器、Inspector、Dock、导出插件和代码生成。
14. [最佳实践、维护与测试](maintenance/index.md)：项目落地建议、分层边界、测试和文档维护。
15. [FAQ](faq.md)：高频分层、包、Wiki 和文档结构问题。
...
- [逻辑空间查询与相关包](standard/input-flow/spatial-query.md)

### Packages

- [官方包总览与包规范](packages/index.md)
- [Capability 能力组合](packages/capability/index.md)
- [Interaction 与 Feedback](packages/interaction-feedback/index.md)
- [Combat 战斗通用能力](packages/combat/index.md)
- [ActionQueue 表现动作队列](packages/action-queue/index.md)
- [Network 与 TurnBased](packages/network-turnbased/index.md)
- [Flow、Domain 与 Physics](packages/flow-domain-physics/index.md)
- [Save 场景存档图](packages/save-graph/index.md)
- [Level、BehaviorTree 与 Quest](packages/level-behaviortree-quest/index.md)
