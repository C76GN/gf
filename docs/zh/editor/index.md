...
本页聚焦 GF 编辑器插件提供的工具入口：AutoLoad 注册、ProjectSettings、脚本模板、扩展管理、菜单/Dock/Inspector、导出插件、访问器生成和项目常量生成。
...
`GF` 是核心插件固定提供的底部统一入口。它把 GF 自带的扩展管理、存档查看、信号图等面板收束到一个响应式工作区，避免多个 GF 面板挤占 Godot 底部栏。标准库和启用扩展贡献的 `editor_dock_paths` 会作为工作区页面加入，而不是各自注册成独立底部标签。

工作区顶部提供自动换行的页面入口，窗口宽度不足时页面按钮会自然平铺到下一行；内容区仍只显示当前页面，避免多个工具同时挤压。工作区只保留足够显示标题、页面入口和“关于”按钮的折叠最小高度；每个页面都会放进无最小高度的裁剪容器，底部面板剩余高度交由 Godot 编辑器控制，页面内容不会覆盖下方底部栏。右上角的“关于”按钮会打开 GF Framework 简介，并提供项目地址、正式文档地址和维护者联系方式。默认保持底部工作区形态，适合跟随 Godot 当前场景、Inspector 和脚本编辑上下文；如果未来需要更重的工具，可以在同一页面记录模型上扩展弹出当前页，而不需要恢复多个分散底部标签。
...
这些类都位于 `kernel/editor`，只定义协议，不知道标准库或官方扩展的具体类型。标准库、官方扩展、社区扩展和项目插件都可以复用这套拆分，让 UI 按钮只负责触发动作，真正修改资源或节点的逻辑集中到命令中，并自然接入 Godot UndoRedo。
...
标准库自带编辑器增强集中声明在 `addons/gf/standard/editor/gf_standard_editor_extensions.gd`，再由根插件 `addons/gf/plugin.gd` 作为组合入口传给 `kernel/editor` 辅助脚本。这些底部编辑器能力会进入 `GF` 工作区页面：

- Node State Machine Inspector。
- Pattern2D Inspector。
- Save Viewer 页面。
- `GFSignalGraphDock` 页面，用于查看当前编辑场景的信号连接图快照。
- BuildInfo 导出插件。

官方扩展或社区扩展的编辑器增强则由各自 manifest 声明：
...
- 只服务某个可选扩展的 Inspector、菜单动作、Dock 或访问器扩展，应放进该扩展的 `editor` 目录并写入 manifest。
