...
`GF Workspace` 是核心插件固定提供的独立编辑器窗口。它把 GF 自带的扩展管理、输入映射、存档文件查看、场景存档图、信号图和诊断快照等面板收束到一个响应式工作区，避免多个 GF 面板挤占 Godot 底部栏，也给 Flow 这类复杂工具保留足够横向空间。标准库和启用扩展贡献的 `editor_dock_paths` 会作为工作区页面加入，而不是各自注册成独立底部标签。
...
- Signal Graph 页面，用于查看当前编辑场景的信号连接图快照和显式开启后的运行事件。
