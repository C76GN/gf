...
行为树节点状态包含 `FRESH`、`SUCCESS`、`FAILURE`、`RUNNING` 和 `ABORTED`。`Sequence` / `Selector` 会在子节点返回 `RUNNING` 时保留当前子节点索引，下次 `tick()` 从该位置继续；返回终态后会重置索引。除了最基础节点外，GF 还提供 `Parallel`、`RandomSelector`、`RandomSequence`、`Inverter`、`AlwaysSucceed`、`AlwaysFail`、`Probability`、`Cooldown`、`TimeLimit`、`Limit`、`Repeat`、`UntilSuccess` 和 `UntilFail`，但它们依然只是代码层控制流组合，不绑定编辑器树或业务类型。`RandomSelector`、`RandomSequence` 和 `Probability` 可在构造时传入 `RandomNumberGenerator`，也可从 `Runner.blackboard["rng"]` 读取随机源，方便固定种子测试、回放或模拟；不提供随机源时保持普通随机行为。`Probability` 会在一次运行周期内锁定是否允许子节点继续执行，避免正在 `RUNNING` 的行为被下一帧重新抽概率中断；`Cooldown.reset()` 只重置运行态，不清空已经开始的冷却，需要手动清除时调用 `clear_cooldown()`。

`GFBehaviorTree.Runner` 默认会通过 `duplicate_runtime()` 复制一份运行树，保留节点配置、子树结构、回调和随机源状态，但不共享 `RUNNING` 游标、调试计数或最近状态。因此同一棵配置树可以安全交给多个 agent 各自创建 Runner；如果项目已经自己为每个 agent 构造独立树，也可以在构造 Runner 时关闭复制。内置节点都会复制自身运行态；项目自定义 `BTNode` 若没有重写 `duplicate_runtime()`，默认会保留原实例以确保重写的 `tick()` 不丢失。自定义节点只要持有独立运行态、游标或缓存，就应重写 `duplicate_runtime()` 并返回自身类型的新实例。`GFBehaviorTree.Runner.get_debug_snapshot()` 会输出根节点状态、tick 次数、耗时和黑板键，`GFBehaviorTree.build_debug_snapshot(node)` 可对任意节点生成同类快照。每个节点的 `tick()` 会记录自己的最终状态，`reset()` 只重置运行游标、计数器或装饰器轮次，不清空最近调试状态；需要重置调试面板或测试断言时，调用节点的 `clear_debug_state()` 或 Runner 的 `clear_debug_state()`。需要分层黑板时，可以使用 `GFBehaviorTree.BlackboardScope` 在父级值上叠加局部覆盖，再把 `to_dictionary()` 结果交给既有节点。GF 仍不负责行为树资源化、可视化编辑、异步任务取消或黑板字段规范，这些应由项目工具或可选扩展按需提供。
