...
槽位 UI 不应在 `slot_changed`、`slot_state_changed`、`slot_filled`、`slot_emptied`、`item_added` 或 `item_removed` 的同步回调里继续修改同一个库存模型；GF 会拒绝这种重入修改，避免第一个监听器排序或移动槽位后，第二个监听器收到的上下文已经被改写。需要“物品移除后自动整理背包”时，把整理延迟到当前通知结束后：
...
`slot_state_changed(slot_index, before_stack_data, after_stack_data)` 会携带变化前后的稳定快照；`slot_filled` 只在空槽变为有内容时发出，`slot_emptied` 只在有内容变为空槽时发出。默认 `sort_slots()` 会把非空槽位前移，并按 `item_id` 和原槽位索引保持稳定顺序；项目可以传入一次性比较回调，或在子类中重写 `_should_sort_slot_before()`。`get_index_debug_snapshot()` 中 `stack_count_by_item` 表示每个物品占用的堆叠数量，`slot_indices_by_item` 才是物品所在的槽位索引列表。
