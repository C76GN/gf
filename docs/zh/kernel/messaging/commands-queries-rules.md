...
当局部架构回退到父级 transient 工厂时，新对象会注入发起解析的局部架构；singleton 工厂则始终注入拥有该绑定的架构。这使全局工厂定义可以服务局部场景，同时保留单例对象的稳定归属。singleton 工厂被替换、注销或随架构销毁时，缓存实例会清理 owner 事件监听、执行 `dispose()`（如果存在）并释放依赖作用域。
