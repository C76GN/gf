...
从 `3.0.0` 起，启用的 GF 扩展也可以通过 manifest 的 `installer_paths` 提供 Installer。当 `gf/extensions/auto_install_enabled_installers` 为 `true` 时，`Gf` 会先按依赖优先顺序执行启用扩展的 Installer，再执行 `gf/project/installers` 中的项目 Installer。这样扩展负责装配自己的抽象模块，项目负责装配业务模块或覆盖绑定。GF 内置扩展安装器只注册扩展级生命周期服务，例如 Save 扩展的 `GFSaveGraphUtility`、Combat 扩展的 `GFCombatSystem` 和 `GFSkillTargetingUtility`；纯数据对象、Resource、命令和场景节点仍由项目或局部上下文按需要创建。
