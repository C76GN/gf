...
可选发送者路径；为空时使用当前节点。 可选碰撞形状配置。设置后可自动生成或更新 CollisionShape2D 子节点。
...
可选发送者路径；为空时使用当前节点。 可选碰撞形状配置。设置后可自动生成或更新 CollisionShape3D 子节点。
...
可选业务接收节点路径；为空时由当前 HurtBox 直接接收。 可选碰撞形状配置。设置后可自动生成或更新 CollisionShape2D 子节点。
...
可选业务接收节点路径；为空时由当前 HurtBox 直接接收。 可选碰撞形状配置。设置后可自动生成或更新 CollisionShape3D 子节点。
...
默认发射数量。 总扩散角度（度）。数量大于 1 时在该范围内均匀分布。
...
可选生成父节点路径。为空时优先使用发射器父节点。 是否在生成后调用发射体的 launch(context)。
