...
单批最大事件数。 本地队列最大事件数。 是否自动附加运行环境上下文。
...
- `_options`: 请求选项 Dictionary；常见字段包含 volume_db、pitch_scale、owner、channel、follow_source 和 metadata。
...
音频资源路径。`stream` 为空时使用该路径加载。 音频流资源。
...
低于该阈值的输入会被视为 0。 达到该阈值时视为满幅输入。 是否把死区外的剩余范围重新映射到 0..1。
...
轴输入检测阈值。 正式接收输入前的倒计时。可用于改键界面避开确认按钮本身。
...
输入低于该长度时视为空闲。 空闲时是否回到 initial_position。
...
每个状态组保留的历史状态名数量。 push_state 可叠加的最大栈深度。 状态组共享黑板。框架不解释其中字段。
...
当前场景资源路径。 与当前场景相邻、可能被提前预热的场景资源路径。
...
默认相邻搜索半径；0 表示只使用固定预加载路径。 单次计划最多返回的临时相邻场景数量；0 表示不限制。 始终参与预加载计划的固定场景路径。
...
目标场景路径。 可选 loading scene 路径。 切换前是否先发起预加载。
...
loading scene 最短保留秒数；为 0 时不额外等待。 项目自定义参数。
...
面板场景路径。 目标 UI 层级。默认使用 GFUIUtility.POPUP。
...
权重；小于等于 0 的条目不会被选择。 项目层可选元数据，框架不解释其含义。
