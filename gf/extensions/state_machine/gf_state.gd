# gf/extensions/state_machine/gf_state.gd

## GFState: 纯代码状态机的状态抽象基类。
##
## 继承自 RefCounted，不依赖 Node 树，可在任何逻辑层使用。
## 通过持有对所属 GFStateMachine 的引用，间接访问框架的
...
## 持有对所属状态机的引用，用于访问框架上下文和切换状态。
var _machine: GFStateMachine
...
func get_model(model_type: Script) -> Object:
	return _machine.get_model(model_type)
...
func get_system(system_type: Script) -> Object:
	return _machine.get_system(system_type)
...
func get_utility(utility_type: Script) -> Object:
	return _machine.get_utility(utility_type)
...
func change_state(state_name: StringName, msg: Dictionary = {}) -> void:
	_machine.change_state(state_name, msg)
