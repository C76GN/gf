...
- `GFUtility`: runtime services such as storage, snapshot history, save graph composition, resource loading, remote caching, scene transitions, build info, time, object pools, UI stack handling, audio, input, logging, and diagnostics.
...
- `GFUtility`：工具层，提供存档、快照历史、资源加载、远程缓存、场景切换、构建信息、时间控制、对象池、UI 栈、日志等通用能力。
