...
1.  **脚本元注解**: `@tool`, `@icon` 等。
2.  **文件级文档注释**: `##` 注释，说明该类的核心职责。(与上一条之间需要空行；如果没有第一条，则直接位于文件顶部)
3.  **`class_name`**
4.  **`extends`**
5.  **`signal` 声明**
6.  **`enum` 定义**
...
12.  **Godot 生命周期 / 回调方法 (Lifecycle & Callbacks)**: 按逻辑执行顺序排列。包括 `_ready()`、`_process()` 等节点生命周期方法，以及 Editor 插件、Inspector 插件等 Godot 类型约定的 `_can_handle()`、`_parse_begin()` 等回调。
	 ...
	 *   ...等等
13.  **公共方法**
14.  **可重写钩子 / 虚方法**：供子类重写、但不作为普通调用入口的方法。若这类方法使用 `_` 前缀，必须集中放在明确的钩子区，而不要混在公共方法区。
...
*   不再要求使用单独的文件路径注释；脚本头部说明统一由文件级文档注释承担。
