...
func test_input_context_get_context_id_prefers_exported_id() -> void:
	var ctx := GFInputContext.new()
	ctx.context_id = &"gameplay"
	assert_eq(ctx.get_context_id(), &"gameplay")


func test_input_context_get_display_name_prefers_exported_name() -> void:
	var ctx := GFInputContext.new()
	ctx.context_id = &"menu"
	ctx.display_name = "主菜单"
	assert_eq(ctx.get_display_name(), "主菜单")


func test_input_context_get_display_name_falls_back_to_context_id_string() -> void:
	var ctx := GFInputContext.new()
	ctx.context_id = &"dialogue"
	assert_eq(ctx.get_display_name(), "dialogue")


func test_input_context_get_context_id_falls_back_to_resource_path() -> void:
	var ctx := GFInputContext.new()
	ctx.take_over_path("res://tests/gf_core/input_context_unit.tres")
	assert_eq(ctx.get_context_id(), StringName("res://tests/gf_core/input_context_unit.tres"))


func test_input_context_display_name_falls_back_to_resource_basename() -> void:
	var ctx := GFInputContext.new()
	ctx.take_over_path("res://tests/gf_core/gameplay_input.tres")
	assert_eq(ctx.get_display_name(), "Gameplay Input")
