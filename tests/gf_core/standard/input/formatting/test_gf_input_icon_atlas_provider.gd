...
const GFInputIconAtlasProviderBase = preload("res://addons/gf/standard/input/formatting/gf_input_icon_atlas_provider.gd")
...
func test_input_icon_atlas_resolves_explicit_key_path() -> void:
	var provider := GFInputIconAtlasProviderBase.new()
	provider.set_icon_path(&"key:k", "res://icons/keyboard/k.png")

	var path := provider.get_event_icon_path(_make_key_event(KEY_K), { "allow_missing_paths": true })
...
func test_input_icon_atlas_builds_template_path() -> void:
	...
	var path := provider.get_event_icon_path(_make_mouse_event(MOUSE_BUTTON_LEFT), { "allow_missing_paths": true })
...
func test_input_icon_atlas_splits_modifier_rich_text() -> void:
	...
	var event := _make_key_event(KEY_K)
	event.ctrl_pressed = true
	event.shift_pressed = true

	var rich_text := provider.get_event_rich_text(event, { "allow_missing_paths": true, "icon_size": 16 })
