...
func test_tile_metadata_layer_paints_queries_and_erases_values() -> void:
	...
	assert_true(GFVariantData.to_bool(layer.get_cell_value(Vector2i(1, 0), &"blocked")), "字段值应可读取。")
...
func test_tile_metadata_layer_merges_schema_and_converts_cache() -> void:
	...
	var cache: GFTileMapCache = layer.to_tile_map_cache()
	var restored: GFTileMetadataLayer = GFTileMetadataLayer.new()
	restored.from_tile_map_cache(cache)

	assert_eq(GFVariantData.get_option_int(layer.get_schema_entry(&"cost"), "default"), 1, "schema 应以副本形式保存。")
	assert_eq(GFVariantData.to_int(restored.get_cell_value(Vector2i(2, 3), &"cost")), 5, "转换为缓存再恢复应保留元数据。")
	assert_eq(GFVariantData.to_text(restored.get_cell_value(Vector2i(2, 3), &"tag")), "road", "恢复后应保留所有字段。")
